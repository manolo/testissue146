...
public class GQuery implements Lazy<GQuery, LazyGQuery> {
    ...
    public static enum FilterStrategy { DEFAULT, OLD, NEW_FALSE, NEW_TRUE, PREDICATE, SIZZLE }
    public static FilterStrategy filterStrategy = FilterStrategy.DEFAULT;
    /**
...
//        GQuery.console.log("filtering " + filters + " using strategy " + filterStrategy);
        switch (filterStrategy) {
            case OLD: return filterOld(filters);
            case NEW_FALSE: return filterNew(false, filters);
            case NEW_TRUE: return filterNew(true, filters);
            case PREDICATE: return filterPredicate(filters);
            case SIZZLE: return filterSizzle(filters);
            default: return filterDefault(filters);
        }
    }

    // filter implementation on 1.4.3-SNAPSHOT
    public GQuery filterDefault(String... filters) {
        String selector = "";
        ...
        if(selector.isEmpty()) {
            ...
            DivElement ghostParent = null;
            ArrayList parents = new ArrayList();
            ArrayList elmList = new ArrayList();
            Element[] n = this.elements;
            int var7 = this.elements.length;

            for(int e = 0; e < var7; ++e) {
                Element array = n[e];
                if(array != window && array.getNodeName() != null && !"html".equalsIgnoreCase(array.getNodeName())) {
                    elmList.add(array);
                    GQuery i = $((Element)array).parent();
                    Object l = ghostParent;
                    if(!i.isEmpty()) {
                        l = i.last().get(0);
                    } else {
                        if(ghostParent == null) {
                            l = ghostParent = Document.get().createDivElement();
                        }

                        ((Element)l).appendChild(array);
                    }

                    if(!parents.contains(l)) {
                        parents.add(l);
                    }
                }
            }

            JsNodeArray var12 = JsNodeArray.create();
            Iterator var14 = parents.iterator();

            while(var14.hasNext()) {
                Element var13 = (Element)var14.next();
                NodeList var15 = getSelectorEngine().select(selector, var13);
                int var16 = 0;

                for(int var17 = var15.getLength(); var16 < var17; ++var16) {
                    Element el = (Element)var15.getItem(var16);
            ...
            return this.pushStack(var12, "filter", selector);
        }
    }
    ...
    public GQuery filterPredicate(String... filters) {
        String sel = "";
        for (String f : filters) {
            sel += (!sel.isEmpty() ? "," : "") + f;
        }
        Predicate filterFn = asPredicateFilter(sel);
        if (filterFn == null) return filterDefault(filters);
        return filter(filterFn);
    }

    private RegExp simpleAttrFilter = RegExp
            .compile("\\[([\\w-]+)(\\^|\\$|\\*|\\||~|!)?=?[\"']?([\\w\\u00C0-\\uFFFF\\s\\-_\\.]+)?[\"']?\\]");
        ...
        return new Predicate() {
            ...
            public boolean f(Element e, int index) {
                switch (op) {
                    ...
                    case '=':
                        return e.getAttribute(attrName).equals(matchVal);
                    case '^':
                        return e.getAttribute(attrName).startsWith(matchVal);
                    case '$':
                        return e.getAttribute(attrName).endsWith(matchVal);
                    case '*':
                        return e.getAttribute(attrName).contains(matchVal);
                    case '|':
                        return (e.getAttribute(attrName) + "-").startsWith(matchVal + "-");
                    case '~':
                        return (" " + e.getAttribute(attrName) + " ").contains(" " + matchVal + " ");
    ...
    public GQuery filterSizzle(String... filters) {
        String sel = "";
        for (String f : filters) {
            sel += (!sel.isEmpty() ? "," : "") + f;
        }
        ...
        for (Element element : elements())
            seed.push(element);
        JsNodeArray array = matches(sel, seed).cast();
        return pushStack(array, "filter", sel);
    }
